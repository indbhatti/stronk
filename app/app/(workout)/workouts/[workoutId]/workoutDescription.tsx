...
export default function WorkoutDescription({
  ...
  useEffect(() => {
    if (workoutDescription !== description) {
      setIsEditing(true);
    } else {
      setIsEditing(false);
    }
  ...
  return (
    <form onSubmit={handleSubmit}>
      <div className="flex flex-col items-end justify-end my-8 dark:text-white px-10">
        {isEditing ? (
          <>
            <div className="px-4 dark:text-gray-500">Description</div>
            <textarea
              value={workoutDescription}
              onChange={handleChange}
              rows={3}
              className="text-end border-0 rounded-lg
        text-xl dark:text-white bg-transparent w-full"
            />
          </>
        ) : (
          <>
            <div className="px-4 dark:text-gray-500 flex">
              Description
              <svg
                viewBox="0 0 24 24"
                fill="none"
