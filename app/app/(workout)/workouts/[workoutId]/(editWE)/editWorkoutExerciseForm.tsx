...
import { editWorkoutExercise, IExerciseData } from "@/serverActions/exercise";
import Popup, { PopupProps } from "../../popup";
import { IWorkoutExercise } from "@/Types/models";

export default function EditWorkoutExerciseForm({
  wExercise,
  onClose,
}: {
  ...
  const [workoutExercise, setWorkoutExercise] = useState<IExerciseData>({
    exerciseId: wExercise.exercise._id,
    sets: wExercise.sets,
  });
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    setLoading(true);
    if (await editWorkoutExercise(wExercise._id, workoutExercise)) {
      setLoading(false);
      setPopup({
        title: "Success",
            ...
            {workoutExercise.sets.map((set, index) => (
              <div key={index} className="flex my-4 items-center">
                ...
                <input
                  ...
                  value={workoutExercise.sets[index].reps}
                ...
                <input
                  ...
                  step={0.1}
                  name="weight"
                  value={workoutExercise.sets[index].weight}
                  onChange={(e) => handleChange(e, index)}
                  className="bg-transparent border-0 text-xl w-1/2 rounded-lg dark:text-white basis-2/5"
                  placeholder="10.5"
                />
